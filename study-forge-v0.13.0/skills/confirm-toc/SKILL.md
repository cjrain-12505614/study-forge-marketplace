...
description: >
  ...
  ##(N) 순번 매칭을 근거 있게 확정하는 스킬 (v0.5.0).
  주경로는 **재생 없는 산술 확정**(항목 강의시간 ↔ MP3 길이 분할)이고, 플레이어는 하위 제목 채록에만 쓴다.
...
window.__scuTick = setInterval(() => {                       // 2초 간격, 변화만 적재
  ...
  const key = (s.rows.find(r => r.sel)?.t || '') + '|' + (s.v?.f || '');
...
다른 상위 목차의 하위 항목은 패널에 **없다** — 그 항목을 따로 열어야 나온다.
...
2단계에서 플레이어를 열었으면, 이후는 **읽기 작업**이다. 여기서 쓰는 도구 세 가지를 한 번 정의해두면 매 수확이 한 줄로 끝나고, 장시간 강의에서 전환을 놓치지 않는다.
