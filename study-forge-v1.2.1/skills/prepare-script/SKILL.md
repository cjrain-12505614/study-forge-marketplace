...
description: >
  ...
  목차가 아직 확정되지 않았으면 confirm-toc 스킬을 먼저 실행한다 (v0.5.1).
version: 0.5.1
---
...
---
# 첫 번째 주제
## (1)첫 번째 주제 실습      ← 영상 384초 = ….mp3 384.40초
---
# 두 번째 주제
## (2)두 번째 주제 실습      ← 영상 402초 = ….mp3 402.69초
---
# 평가하기
## 학습평가
---
...
- `## (N)서브제목` → 강의노트의 `## (N)서브제목`. `(N)`은 영상 파일의 순번과 대응 — **닫는 괄호 뒤에는 공백을 넣지 않는다**(기존 노트 표기)
...
- `# 들어가기`, `## 학습개요`, `# 평가하기`, `## 학습평가`, `# 정리하기`, `## 학습정리` → 영상 자료가 없어도 강의노트에 반드시 포함되는 필수 섹션
  - 내용의 출처는 수강 중 채록(`confirm-toc` 6단계)이다 — **학습평가는 `정답확인` 채점까지 해서 회수한 공식 정답·공식 해설을 포함**한다(2026-09-17 사용자 상시 정책). 「(추정)」 정답은 금지
...
**번호 없는 항목이 앞에 올 수 있다.** 매 주차 첫 영상은 **`인트로`(타이틀 영상)** 이고, 여기에는 **MP3가 없다.** ⚠️ **길이로 식별하지 말 것** — `doRandomIntroMedia.scu` 공용 랜덤 클립이라 로드마다 다르다(실측 13.11초, 구 문서 범위 18~22초 밖). 판별 기준은 `currentSrc`뿐이다. 목차상 첫 항목이지만 첫 MP3(`_01`)는 그다음 항목에 붙는다.
...
- `## (N)` 형식의 영상 대응 항목이 있는지
- `---` 가로선이 `#` 그룹 사이에 있는지
- 관례 섹션(`# 들어가기`/`## 학습개요`, `# 평가하기`/`## 학습평가`, `# 정리하기`/`## 학습정리`)이 포함되어 있는지

⚠️ **없다고 해서 오류가 아니다 — 목차가 정본이다.** 그 주차에 실제로 없는 항목을 검증기가 「누락」이라 부르면 없는 항목을 지어내게 된다.

- AWS클라우드실습프로젝트 2주차는 강의 항목이 하나뿐이라 **`평가하기`·`정리하기`가 아예 없다**
- `학습개요`는 과목에 따라 슬라이드가 아니라 **영상**이라 `## (2)학습개요`처럼 번호를 갖는다 (GitHub포트폴리오 2주차)

따라서 결과는 「확인 필요」로만 보고하고 **자동 추가하지 않는다.** 실제 강의실 목차와 대조해 없는 것이면 그대로 둔다.
...
def validate_outline(outline_text):
    ...
    # ⚠️ 완전 일치로 검사하면 안 된다 — 실제 목차 줄에는 확정 근거 주석이 붙는다
    #    예: `## 학습평가                      ← 퀴즈 페이지 · MP3 없음`
    required_h1 = ['들어가기', '평가하기', '정리하기']
    required_h2 = ['학습개요', '학습평가', '학습정리']

    def head_text(line, level):
        s = line.strip()
        if not s.startswith('#' * level) or s.startswith('#' * (level + 1)):
            return None
        title = re.split(r'\s*←', s[level:])[0].strip()
        # `## (2)학습개요`처럼 번호가 붙어 있어도 같은 항목이다 —
        # 학습개요는 과목에 따라 슬라이드가 아니라 영상이라 번호를 갖는다
        return re.sub(r'^\(\d+\)\s*', '', title)

    for req in required_h1:
        if not any(head_text(l, 1) == req for l in lines):
            issues.append(f"확인 필요: # {req} 없음")
    for req in required_h2:
        if not any(head_text(l, 2) == req for l in lines):
            issues.append(f"확인 필요: ## {req} 없음")
...
def build_script_file(course_name, week, outline_text):
    ...
    # 학습목차에서 `## (N)제목` 항목 추출 — `##` 뒤 공백이 없는 구형 표기도 함께 받는다
    # ⚠️ 목차 줄 꼬리의 `← 영상 N초 = ….mp3 N초` 주석은 본문 제목으로 옮기지 않는다
    numbered_sections = [
        re.split(r'\s*←', m)[0].rstrip()
        for m in re.findall(r'(?m)^(##[ \t]*\(\d+\).+)$', outline_text)
    ]
    # 번호 없는 채록 구역도 본문 구역에 자리를 만들어 둔다 —
    # 학습개요·학습평가·학습정리는 영상·MP3가 없어 수강 중 채록(학습평가는 정답확인 채점 포함)만이 유일한 출처다
    captured = [
        re.split(r'\s*←', m)[0].rstrip()
        for m in re.findall(r'(?m)^(##[ \t]*(?:학습개요|학습평가|학습정리).*)$', outline_text)
    ]

    # 강의녹음 스크립트 구역 생성
    script_section = f"<{course_name} {week}주차 강의녹음 스크립트>\n\n"
    for section in numbered_sections + captured:
