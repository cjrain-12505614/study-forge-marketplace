...
> | **4** | 강의교안 PDF → 이미지 변환 + 읽기 | `pdf2image` dpi=150, 10~15페이지씩 Read |
...
> | **8** | 시각 자료 생성 | HTML → Playwright → PNG (NanumGothic `@font-face`, SVG 금지) |
...
```bash
pip install pdf2image pdfminer.six --break-system-packages
...
- whisper-mcp 실행 후에는 VM 재시작/마운트 갱신이 발생할 수 있으므로, **경로를 캐시하지 말고 매번 `os.listdir()`로 재획득**해야 한다
...
- `# 평가하기` → `## 학습평가` (교안의 학습 평가 문항이 있으면 기록, 없으면 빈 섹션)
- `# 정리하기` → `## 학습정리` (교안의 학습 정리 내용이 있으면 기록, 없으면 빈 섹션)
...
def convert_lecture_pdf(pdf_path, week):
    """강의교안 PDF를 JPEG 이미지로 변환"""
...
def extract_practice_text(pdf_path, week):
    ...
    images = convert_from_path(temp, dpi=150)
    saved = []
    for i, img in enumerate(images):
        path = os.path.join(output_dir, f'page_{i+1:03d}.jpg')
        img.save(path, 'JPEG', quality=85)
    ...
    print(f"실습 PDF 이미지 변환 완료: {len(images)}페이지 → Read 도구로 시각적 읽기 필요")
...
- `## 학습평가` → 교안 마지막 부분의 평가 문항 (있는 경우에만)
- `## 학습정리` → 교안 마지막 부분의 학습 정리 (있는 경우에만)
...
# 한글 폰트 — NanumGothic (최초 1회)
pip install koreanize-matplotlib --break-system-packages
mkdir -p ~/.fonts
cp ~/.local/lib/python3.10/site-packages/koreanize_matplotlib/fonts/NanumGothic*.ttf ~/.fonts/
fc-cache -f
```

> **왜 이 방식인가?** VM 환경에 한글 폰트가 없어 SVG `<text>`의 한글이 □로 깨진다.
> Google Fonts CDN은 네트워크 제한으로 접근 불가, `@fontsource/noto-sans-kr` npm 패키지도 설치되지 않는다.
> `koreanize-matplotlib`가 번들하는 NanumGothic.ttf를 로컬에 설치하는 것이 유일한 방법이다.
...
# ── 폰트 설정 ──
FONT_REGULAR = os.path.expanduser('~/.fonts/NanumGothic.ttf')
FONT_BOLD = os.path.expanduser('~/.fonts/NanumGothicBold.ttf')

FONT_FACE_CSS = f"""
@font-face {{ font-family: 'NanumGothic'; src: url('file://{FONT_REGULAR}'); font-weight: 400; }}
...
body {{ font-family: 'NanumGothic', sans-serif !important; margin: 0; padding: 20px; }}
"""
...
- `wait_for_timeout(1000)`으로 `@font-face` 로딩 시간 확보
- `device_scale_factor=2`로 고해상도 출력
- `--no-sandbox`, `--disable-web-security`로 로컬 `file://` 폰트 접근 허용

### HTML 템플릿 규칙

1. **한글 폰트**: `@font-face`로 로컬 NanumGothic 지정 (CDN·npm 사용 금지)

```html
<style>
  @font-face { font-family: 'NanumGothic'; src: url('file:///root/.fonts/NanumGothic.ttf'); font-weight: 400; }
  @font-face { font-family: 'NanumGothic'; src: url('file:///root/.fonts/NanumGothicBold.ttf'); font-weight: 700; }
  * { font-family: 'NanumGothic', sans-serif !important; }
...
- Google Fonts CDN 사용 불가 (네트워크 제한)
- `@fontsource/noto-sans-kr` npm 패키지 설치 불가
- 반드시 `koreanize-matplotlib`이 번들하는 NanumGothic.ttf를 로컬 `@font-face`로 참조
...
3. **필수 섹션 존재 확인**: `# 들어가기`/`## 학습개요`, `# 평가하기`/`## 학습평가`, `# 정리하기`/`## 학습정리` 존재 여부
...
| 강의교안 내용 추출 실패 | 이미지 기반 슬라이드 | `pdf2image`로 이미지 변환 후 시각적 읽기 |
| pdfminer 텍스트 추출 실패 | 폰트 임베딩 문제 ("Could not get FontBBox") | `pdf2image`로 이미지 변환 fallback (`extract_practice_text` 참조) |
| `pdf2image` 미설치 | 라이브러리 없음 | `pip install pdf2image --break-system-packages` + `apt-get install poppler-utils` |
...
| 한글 텍스트가 □로 표시 | 시스템에 CJK 폰트 없음 | `pip install koreanize-matplotlib` → NanumGothic.ttf를 `~/.fonts/`에 복사 → `fc-cache -f` |
| Google Fonts CDN / @fontsource 로드 실패 | 네트워크 제한 + npm 설치 불가 | `koreanize-matplotlib` 번들 NanumGothic 로컬 `@font-face` 사용 |
...
| **whisper-mcp 후 경로 무효화** | **whisper-mcp 실행이 VM 재시작/마운트 갱신 유발** | **whisper-mcp 완료 후 `os.listdir()`로 경로 재획득** |
