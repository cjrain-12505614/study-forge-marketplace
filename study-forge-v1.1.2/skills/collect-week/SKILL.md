...
name: collect-week
description: >
  SCU 포털에서 해당 주차의 강의교안 PDF와 강의녹음 MP3를 내려받아 과목 폴더에 배치하는 스킬 (v0.2.0). 과목공지 읽기도 담당한다.
...
`go.targetHome`은 **4인자**다. 인자를 줄이면 `TypeError: Cannot read properties of undefined (reading 'match')`로 죽는다.
...
`go`는 **포털 메인(`/ptl/main/Main.scu`)에만 있다.** 강의실 페이지(`/wlms/lecture/main/mainList.scu`)에는 없으므로, 과목을 바꿀 때는 포털 메인으로 먼저 돌아간다. 강의실 페이지에는 과목 전환 UI가 없다.
