...
- **학습평가** — 퀴즈를 풀어야 다음으로 넘어간다. **기본은 문항만 채록하고 답을 클릭하지 않는 것**이다 —
  ...
  **사용자가 명시적으로 대신 풀라고 지시하면** 전사·교안에서 근거를 찾아 답을 확정한 뒤 진행한다 — 찍지 않는다.
  ⭐ **단 노트 작성만이 목적이라면 대신 풀 이유가 없다** — 정답·해설은 `[data-field="explanation"]`에
  **처음부터 들어 있어 클릭 없이 회수된다**(아래 절). 대신 푸는 것이 사는 유일한 경우는
  **사용자가 진도·완료 처리를 함께 맡긴 때**뿐이고, 그때도 ① 이 퀴즈는 배점 항목이 아닌 **형성평가**이며
  ② 그렇게 완료 처리된 항목은 **8단계 보고에 반드시 포함**한다.
...
| 남는 근거는 **진도·완료 처리**뿐이다 | `평가하기`는 퀴즈를 풀어야 다음으로 넘어가므로, 사용자가 그 진행까지 맡긴 경우에만 해당한다 |
...
- 자동 채점으로 완료 처리된 항목은 **8단계 보고에 반드시 포함**한다 — **빈 답안·오답으로 채점된 문항이 생겼으면 그 사실도** 함께 적는다
