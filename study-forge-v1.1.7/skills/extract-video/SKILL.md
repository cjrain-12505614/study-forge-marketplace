...
name: extract-video
description: >
  강의영상 MP4에서 슬라이드 프레임 캡처, 오디오 추출, 다글로(daglo) 전사, 스크립트 TXT 생성을 수행하는 스킬 (v0.8.1).
...
강의영상 MP4에서 슬라이드 프레임 캡처, 오디오 추출, 다글로(daglo) 전사를 수행하고, 결과를 스크립트 TXT의 `## (N)` 구역에 자동 삽입한다.

> ## ⛔ 영상 = **자막(본문)** + **프레임(화면)** — 오디오 전사는 폴백이다
>
> 같은 영상 항목에 **사람이 만든 공식 자막**이 `video.textTracks`로 실려 있고, 다글로 전사보다 정확하다(이산값 4/4 정확 · 규범 표기 40:0 · 다글로는 +9.7%가 중복 발화·필러). 자막은 `confirm-toc`이 무료·무업로드로 수확한다.
>
> | 항목 상태 | 본문 | 화면 | 이 스킬에서 도는 단계 |
> |---|---|---|---|
> | 자막 있음 | **자막(정본)** | 프레임 | **프레임 캡처만** — 오디오 추출·업로드·승인·전사·후처리 전부 건너뜀 |
> | 자막 없음(cue 0) | 다글로 전사 | 프레임 | 전 단계 수행 |
>
> **자막이 확보된 항목에서 이 스킬이 하는 일은 프레임 캡처 하나다.** 프레임의 역할도 '본문 교정 근거'가 아니라 **화면 정보 보강**으로 좁아진다.
...
def start_transcription_for_week(meta_map, topic='IT'):
    ...
    for i in range(0, len(ids), 5):
        mcp__daglo__start_transcription(
            ...
            language='ko-KR',
            topic=topic,                # ⚠️ 서버 enum. 과목명 등 임의 문자열은 400 → 'IT'
            use_dictionary=True,
...
**서버가 값을 검증하는 두 인자** — `file_meta_ids`는 **요청당 5개 상한**, `topic`은 **서버 enum**(`IT` 등)이다. 과목이 무엇이든 `topic='IT'`를 쓰고, 전공 용어 인식률은 **다글로 웹 계정 사전 등록 + `use_dictionary=True`** 로 올린다.
...
def insert_transcripts_into_script(script_path, transcripts):
    ...
    for seq in sorted(transcripts.keys()):
        ...
        # `## (N)제목` 바로 뒤(다음 줄)에 삽입 — `##` 뒤 공백이 없는 구형 표기도 함께 받는다
        pattern = rf'(?m)^(##[ \t]*\({seq}\)[^\n]*\n)'
        # ⚠️ 전사 본문을 치환 템플릿에 넣으면 `\d`·`\1` 같은 escape가 재해석된다 → 함수 치환으로 삽입
        after, n = re.subn(pattern, lambda m: m.group(1) + text + '\n', after, count=1)
        (inserted if n else missed).append(seq)

    if missed:
        # 치환 0건을 성공으로 보고하면 전사 결과가 조용히 사라진다
        print(f"❌ 삽입 실패 {len(missed)}개 순번: {missed} — 스크립트의 `## (N)제목` 표기를 확인하라")
        return False
    ...
    print(f"전사 텍스트 삽입 완료: {len(inserted)}개 순번")
    return True
```
...
### ⚠️ 영상 성격부터 판별한다 — 방식이 다르다

| 영상 성격 | 특징 | 추출 방식 |
|---|---|---|
| **슬라이드 강의** | 교안 슬라이드 + 강사 화면 오버레이 | 장면 전환 감지 `gt(scene,0.02)` |
| **라이브 데모** | 코랩·IDE 화면 녹화, 타이핑·스크롤 연속 | **고정 간격 `fps=1/15`** |

**라이브 데모에 장면 검출을 쓰면 안 된다.** 화면이 조금씩만 바뀌어 점수가 오르지 않고, 조작 과정을 통째로 놓친다.

### 방법 A: 장면 전환 감지 (슬라이드 강의)

```bash
ffmpeg -i "영상파일.mp4" -vf "select='gt(scene,0.02)'" -vsync vfr \
  -pix_fmt yuvj420p "frames/frame_%04d.jpg" -y
```

```python
import subprocess, os

def capture_slide_frames(video_path, seq, week, threshold=0.02):
    ...
    cmd = [
        ...
        '-vsync', 'vfr',
        '-pix_fmt', 'yuvj420p',      # 일부 영상에서 mjpeg 인코더 실패 방지
...
**임계값 0.3은 너무 높다.** 강의 영상은 슬라이드 위에 강사 화면·자막이 겹쳐 있어 전환 점수가 낮게 나온다. 실측에서 **22분 영상이 0.3으로 단 1장**만 검출됐고, `0.02`로 낮추자 15장이 나왔다.

- 슬라이드 강의 기본값 — **`0.02`**
- 너무 많이 잡히면 `0.05`~`0.1`로 올린다

### 방법 B: 고정 간격 (라이브 데모 · 장면 검출 실패 시)

```bash
ffmpeg -i "영상파일.mp4" -vf "fps=1/15" -pix_fmt yuvj420p "frames/frame_%04d.jpg" -y
```

- 코랩 실습처럼 화면이 연속 변화하는 영상은 **이쪽이 정답** (7분 40초 → 31장)
- 긴 슬라이드 영상의 보조 수단으로는 `fps=1/30`

### ⚠️ 추출 후 장수를 확인하고 판단할 것

**장수를 보지 않고 다음 단계로 넘어가지 않는다.** 영상 길이 대비 결과가 빈약하면 방식이 잘못된 것이다.

```python
mins = duration_sec / 60
if len(frames) < mins / 3:        # 3분에 1장도 안 되면 이상
    print(f"⚠️ {mins:.0f}분 영상에 {len(frames)}장 — 임계값을 낮추거나 fps 고정으로 전환")
```

| 증상 | 판단 | 조치 |
|---|---|---|
| 22분 영상에 1~2장 | 임계값 과다 | `0.02`로 낮춘다 |
| 화면 녹화인데 몇 장 안 나옴 | 장면 검출 부적합 | `fps=1/15` 고정 간격 |
| 수백 장 | 임계값 과소 / 카메라 영상 | `0.05`~`0.1`로 올린다 |
...
> **핵심 변경 (v0.8.0)**: 프레임 추출을 영상 성격별로 분기 — 슬라이드 강의는 scene 임계값 `0.02`(기존 0.3은 22분 영상에서 1장만 검출), 라이브 데모는 `fps=1/15` 고정 간격. 추출 후 장수 확인을 의무화. 다글로 서버 제약(`file_meta_ids` 5개 상한, `topic` enum, `참석자 N:` 라벨) 반영.
>
...
| 전사문에 `참석자 1:` 접두 | **`use_speaker_diarization=False`로도 붙는다** | 후처리 정규식 `^\s*참석자\s*\d+\s*[:：]\s*` 로 제거 |
| `Maximum of 5 fileMetaIds are allowed` | 한 요청에 6개 이상 전달 | 5개씩 분할 호출 |
| `topic must be a valid enum value` | `topic`에 과목명 등 임의 문자열 | `IT` 사용 |
| 프레임이 1~2장만 나옴 | scene 임계값 0.3이 과다 | `0.02`로 낮추거나 `fps=1/15` (8단계 참조) |
| mjpeg 인코더 실패 | 픽셀 포맷 불일치 | `-pix_fmt yuvj420p` 지정 |
