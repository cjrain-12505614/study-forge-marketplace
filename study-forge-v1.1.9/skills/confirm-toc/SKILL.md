...
- → **`정리하기` 채록에 퀴즈 풀이는 필요 없다.** `›`가 멈추면 **풀지 말고 기다렸다가 다시 시도**하거나,
  강의실에 재진입해 `평가하기`의 완료 표시를 확인한 뒤 다시 연다
- ⛔ 「막혔다 → 퀴즈를 풀어야겠다」로 넘어가지 말 것. **불필요하게 대신 푸는 경로로 빠지는 함정이다**
...
window.__scuIdle = function () {
  ...
  for (const d of docs) for (const p of d.querySelectorAll('.eco-popup')) {
    ...
    if (r.width && r.height && /장시간 사용하지 않아/.test(t)) return t;   // 보이는 팝업만
  }
  return null;
};
...
- **학습평가** — 퀴즈 페이지. ⛔ **풀지 않아도 다음으로 넘어간다** — `평가하기`는 슬라이드형이라 **열람만으로 완료**되고,
  그 완료가 반영되면 퀴즈를 한 문항도 풀지 않은 상태에서 `›`로 `정리하기`가 열린다(2026-09-09 GitHub포트폴리오 2주차 실증 ·
  3단계 「자동 전환은 모듈 경계도 넘는다」의 v1.1.5 정정). `›`가 조용히 막히면 **풀지 말고 완료 반영을 기다린다.**
  **기본은 문항만 채록하고 답을 클릭하지 않는 것**이다 — 퀴즈 풀이는 사용자 본인의 행위다. 사용자가 답을 알려주면 정답·해설을 스크립트에 적어둔다.
  **사용자가 명시적으로 대신 풀라고 지시하면** 전사·교안에서 근거를 찾아 답을 확정한 뒤 진행한다 — 찍지 않는다.
  ⭐ **노트 작성도, 진도·다음 항목 이동도 대신 풀 이유가 되지 않는다** — 정답·해설은 `[data-field="explanation"]`에
  **처음부터 들어 있어 클릭 없이 회수**되고(아래 절), 완료와 이동은 위처럼 풀이와 무관하다.
  대신 푸는 것은 **사용자가 풀이 자체를 명시적으로 맡긴 때**뿐이고, 그때도 ① 이 퀴즈는 배점 항목이 아닌 **형성평가**이며
  ② 대신 푼 문항과 그 채점 결과(`Y`/`N`)는 **8단계 보고에 반드시 포함**한다.
...
| O/X형 | `div.component-data-field` 중 `textContent`가 `O`/`X` (`input` 0개) · 번호 배지 `.item-select-option-no`도 2개 있다(`1`=O · `2`=X) — `.item-option-row` 클릭은 **무반응**(3주차) | `span.maui-border-button` |
| **주관식 단답형** | **선택지 없음** (`.item-select-option-no` 0개) · 입력은 `[contenteditable]` | `span.maui-border-button` |
...
#### ⭐⭐ 정답·해설은 처음부터 DOM에 있다 — **누르지 않고 전량 채록한다**

**2026-09-09 GitHub포트폴리오 2주차 실측 (12문항) — 이것이 정답 채록의 정본이다.**

문항 컨테이너 안 **`[data-field="explanation"]`에 정답과 해설이 이미 들어 있다.**
`display:none`으로 숨겨져 있을 뿐이고, `정답확인`은 그것을 **보이게 만들 뿐**이다.

```javascript
// 아무것도 클릭하지 않은 상태에서 전 문항의 정답·해설을 회수한다
[...d.querySelectorAll('[data-type="item"]')].map((it, k) => {
  const ex = (it.querySelector('[data-field="explanation"]')?.textContent || '')
               .replace(/\s+/g, ' ').trim();
  const m = ex.match(/^정답\s*(.*?)\s*해설\s*(.*)$/);     // 「정답…해설…」 한 덩어리로 붙어 나온다
  return {
    k: k + 1,
...
- ⭐ **클릭이 전혀 필요 없다** — 12문항 전량을 `정답확인` 한 번 누르지 않고 채록했고,
  그때 모든 문항의 `data-passyn`은 `null`(미채점)이었다
- ⛔ **따라서 「대신 풀어야 정답을 얻는다」는 정당화는 성립하지 않는다.** 기본 규칙대로
  **문항·정답·해설만 채록하고 풀이는 사용자에게 남긴다**
...
→ **`[data-field="explanation"]`을 신뢰해도 된다.** 누르지 않고 채록한 값으로 노트를 써도 안전하다.
...
#### 참고 — 채점 결과 마커 (직접 풀 때만 의미가 있다)

사용자가 명시적으로 대신 풀라고 지시해 실제로 클릭한 경우, **내가 고른 답의 정오**는 이렇게 읽는다.
...
- 이 **마커들은 `정답확인`을 눌러야 생긴다**(미클릭 문항엔 부재, 실측). 정답 자체는 위 절로 이미
  알 수 있으므로, 이 마커는 **채점 여부·정오 확인 용도**로만 쓴다
...
- 같은 날 앞서 플레이어 항목을 **다시 열었을 때는 채점 상태가 초기화돼 있었다**(관측 1회). 재채점 경로로 확인된 것은 아니며,
  재오픈은 사용자가 할 일이다
...
| 남는 근거는 **사용자의 명시적 지시**뿐이다 | 사용자가 **풀이 자체를** 맡긴 경우에만 해당한다 — 정답 확보·진도·이동은 모두 풀이 없이 해결된다 |
| 이 퀴즈는 배점 항목이 아닌 **형성평가**다 | 대신 풀어도 점수가 걸린 행위가 아니다 — 그래도 찍지 않고 근거로 답한다 |
...
| **주관식 단답형** (GitHub 2주차 11·12번) | 선택지 없음 — `[contenteditable]`에 값을 입력 | `span.maui-border-button` |
...
- 입력 후 `정답확인`은 다른 유형과 동일하게 `span.maui-border-button`을 합성 클릭한다
...
- **동기 호출로 문항마다 나눠 실행한다. 선택·선택 확인·채점도 각각 별도 호출이다**
- 4문항이면 **총 12회 호출** — `pick(0)` → `sel(0)` → `conf(0)` → `pick(1)` → `sel(1)` → `conf(1)` → …
...
- `__scuQuiz.pick()`이 O/X `div.component-data-field`와 그 부모에 pointer·mouse 이벤트를 발사했다 —
...
2. `pick(k, …)` — **선택만** 한다
3. **별도 호출로 `sel(k)`** — 그 문항 컨테이너의 선택 상태를 읽는다. **의도한 보기가 아니면 멈춘다**
4. `sel(k)`가 `null`이면 **합성 이벤트를 반복하지 말고 실제 마우스 클릭으로 전환**한다 —
   `focus(k)`로 문항을 화면 중앙에 두고 → 스크린샷 → 그 문항의 번호 배지를 **즉시** 좌표 클릭 → `sel(k)` 재확인
5. `sel(k)`가 의도한 보기일 때만 `conf(k)` — 헬퍼의 `conf`는 **선택이 없으면 누르지 않고 거부**한다
6. `정답확인`도 합성 이벤트에 반응하지 않으면 같은 방식(스크린샷 → 좌표 클릭)으로 누른다. 누른 뒤 `data-passyn`으로 채점 여부를 확인한다

⛔ **「선택 + 정답확인을 한 호출에」는 이번 사고의 재현 코드다.** 선택이 등록됐는지 볼 틈이 없다.

**헬퍼 정의 — 최상위 프레임에서 1회** (⚠️ 3단계 `__scuIdle`을 먼저 정의한다)

```javascript
window.__scuQuiz = (function () {
  if (!window.__scuIdle) return { err: '__scuIdle 먼저 정의 (3단계 「유휴 세션 종료」)' };
  const docs = [];
  (function collect(d) {
    docs.push(d);
    [...d.querySelectorAll('iframe')].forEach(f => {
  ...
  const sel = k => {
    ...
    if (b) return ((b.closest('.item-option-row') || b).textContent || '').replace(/\s+/g, ' ').trim();
    const ce = it.querySelector('[contenteditable]');                     // 주관식
    return ce && (ce.textContent || '').trim() ? 'text:' + ce.textContent.trim() : null;
  };

  return {
    d, W, real, opts, btns, items, sel,
    n: () => Math.min(opts().length, btns().length) / 2,
    // 문항 k(0-based) 선택 — O = opts[2k] · X = opts[2k+1]. ⚠️ 이 호출은 이벤트를 쏠 뿐이다 — 반드시 sel(k)로 확인
    pick: (k, ox) => {
      if (idle()) return '중단 — 유휴 종료 팝업 (사용자 재오픈 필요)';
      const o = opts(), el = ox === 'O' ? o[2 * k] : o[2 * k + 1];
      return real(el) ? `Q${k + 1} ${ox} 이벤트 발사 — 등록 여부는 sel(${k})로 확인` : `Q${k + 1} 선택지 없음`;
    },
    // 실제 마우스 클릭 전환용 — 문항을 화면 중앙에 둔다 (스크롤은 상태를 바꾸지 않는다)
    focus: k => { const it = items()[k];
                  if (!it) return `Q${k + 1} 없음`;
                  it.scrollIntoView({ block: 'center' });
                  return `Q${k + 1} 중앙 — 스크린샷 후 번호 배지를 즉시 좌표 클릭`; },
    // 문항 k 채점 — ⚠️ 텍스트 필터가 아니라 위치 인덱스 btns[2k] · ⛔ 선택이 없으면 누르지 않는다
    conf: k => {
      if (idle()) return '중단 — 유휴 종료 팝업 (사용자 재오픈 필요)';
      const s = sel(k);
      if (!s) return `Q${k + 1} 선택 미등록 — 정답확인 보류 (실제 마우스 클릭으로 선택 후 재시도)`;
      const b = btns();
      return real(b[2 * k]) ? `Q${k + 1} 정답확인 발사 (선택: ${s}) — data-passyn으로 채점 확인` : `Q${k + 1} 버튼 없음`;
    }
  };
})();
...
**문항별 선택 → 선택 확인 → 채점 — ⚠️ 반드시 한 호출에 한 동작씩**

```javascript
// 호출 ①  __scuQuiz.pick(0, 'X')     ← 근거로 확정한 답만 넣는다 · 선택만 한다
// 호출 ②  __scuQuiz.sel(0)           ← '2X'처럼 의도한 보기가 나와야 한다 · null이면 실제 마우스 클릭으로 전환
// 호출 ③  __scuQuiz.conf(0)          ← 선택이 없으면 헬퍼가 누르지 않고 거부한다
// 호출 ④  __scuQuiz.pick(1, 'O')   … 문항 수 × 3 회
window.__scuQuiz.pick(0, 'X')
```

- 각 호출의 반환 문자열로 **의도한 문항이 처리됐는지 매번 대조**한다
- `sel(k)`은 선택 직후 렌더가 늦을 수 있다 → `null`이면 **1~2초 뒤 한 번 더** 읽고, 그래도 `null`이면 **미등록으로 확정**하고 실제 클릭으로 간다
- 채점 직후에는 렌더가 늦다 → `conf(k)` 다음 **3~4초 기다린 뒤** `data-passyn`을 확인하고 다음 문항으로 간다
- 한 문항이라도 `선택지 없음`·`버튼 없음`이 나오면 **중단하고 유형 분기를 다시 확인한다** — 밀어붙이면 엉뚱한 문항을 누른다
- `중단 — 유휴 종료 팝업`이 나오면 **그 자리에서 아무것도 누르지 않는다** — 사용자에게 **감시 중인 탭에서** 다시 열어 달라고 한다
...
| 유형 | 해설 | 실측 |
|---|---|---|
| **O/X형** | 거짓 진술 문항에만 | 딥러닝 2주차 Q1·Q4(거짓)=해설 / Q2·Q3(참)=없음 · 문제해결 1주차 동일 · 딥러닝 3주차 Q2·Q3(거짓)=해설 / Q1(참)=없음 |
...
- 결과가 0줄이면 **추출 실패**다 — 「정리 없음」으로 보고하지 말고 문서 특정(`.toc-row`)부터 다시 본다
