...
name: transcribe
description: >
  강의녹음 MP3 파일을 다글로(daglo) MCP 서버로 텍스트 스크립트(TXT)로 변환하는 스킬 (v0.8.2).
...
> ## ⛔ 이 스킬은 **폴백이다** — 먼저 자막을 확인한다
>
> **강의실 플레이어에는 사람이 만든 공식 자막이 있고, 그것이 이 스킬의 출력보다 정확하다.** 자막은 `confirm-toc`이 무료·무업로드로 수확한다.
>
> | 실측 대조 (딥러닝 2주차, 같은 음원 10개) | 자막 | 다글로 |
> |---|---|---|
> | 이산값 언급 | **4회 전부 정확** | 3회 + 이상값 오인식 1회 |
> | 경사 하강법 / 손실 함수 / 최솟값 (규범 표기) | **40:0 · 41:0 · 28:0** | 혼입 |
> | 분량 | 기준 | +4,963자(+9.7%) — **중복 발화·필러** |
>
> **실행 전 확인 순서**
> 1. `confirm-toc`으로 자막을 수확했는가 → 확보된 항목은 **전사하지 않는다**(과금 회피)
> 2. 영상형인데 cue가 0인 항목만 이 스킬의 대상이다
> 3. 슬라이드형(학습개요·학습평가·학습정리)은 자막도 MP3도 없다 → `confirm-toc`의 DOM 채록(학습평가는 `정답확인` 채점 포함 — 공식 정답·공식 해설 회수)으로 간다. 전사로는 절대 복원되지 않는다

> **영상이 있는 경우**: `extract-video` 스킬을 사용하세요. 오디오 추출 → 다글로 전사 → `## (N)` 삽입까지 한 번에 처리됩니다.
> **이 스킬(transcribe)은**: 자막이 없고 영상도 없이 MP3만 별도로 존재하는 경우의 폴백 경로입니다.
...
def insert_transcripts_into_script(script_path, transcripts):
    ...
    if missed:
        # 치환 0건은 조용히 지나가면 안 된다 — 제목 표기가 다르거나 순번이 어긋난 것이다
...
> **핵심 변경 (v0.8.2)**: 슬라이드형 안내에 학습평가 `정답확인` 채점(공식 정답·공식 해설 회수, `confirm-toc`)을 명시 — 학습평가 정답은 전사 경로로 얻을 수 없다(2026-09-17 사용자 상시 정책).
>
...
| 상황 | 사용할 스킬 |
|------|------------|
| **강의실에서 자막을 얻을 수 있음** (영상형 항목) | ⭐ **`confirm-toc` — 자막 수확이 첫 분기다.** 무료·무업로드·정본 |
| 강의영상(MP4)이 있고 **자막이 없음** | `extract-video` — 오디오 추출 + 전사 + 프레임 캡처 |
| MP3만 있음 (영상 없음 또는 별도 녹음) · **자막 없음** | `transcribe` — MP3 전사만 수행 |
...
| 전문 용어가 대량 오인식 | `initial_prompt` 부재 (다글로 특성) | ⭐ **1순위: `confirm-toc`으로 자막을 수확해 본문을 통째로 교체한다** — 자막은 규범 표기가 이미 일관(40:0 · 41:0 · 28:0)이라 교정 자체가 불필요하다. 자막이 없을 때만 7단계 LLM 후처리 + 다글로 웹 계정 사전 등록 |
| 후처리했더니 오히려 표기가 틀어짐 | `whisper-terms.txt`를 정답으로 기계 정규화 | ⛔ **용어집은 정답이 아니라 「의심 후보」다.** 딥러닝 용어집의 `에폭`은 틀렸다 — 강의도 자막도 `에포크`(각 16회). **강의 실제 표기가 항상 우선** |
