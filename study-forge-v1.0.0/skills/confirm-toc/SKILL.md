...
- 예외는 **6단계 「퀴즈 자동 채점」 하나뿐**이며, **사용자가 명시적으로 지시했을 때만** 쓴다. 그 외 어떤 도구에도 클릭·재생 코드를 추가하지 말 것
...
window.__scuTick = setInterval(() => {
  try {
    ...
    const it = /평가하기|학습평가|정리하기|학습정리/.test(r.t || '');
    const cap = window.__scuCapture ? window.__scuCapture() : { len: 0 };
    if (it || cap.len > 400) s.cap = cap;
...
- 전환 로거의 스냅샷 조건은 **(선택 항목이 `평가하기`/`정리하기`) 또는 (글자수 > 400)** 으로 둔다
