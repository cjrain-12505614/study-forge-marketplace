...
- 검증 지표: **마지막 cue 종료 시각 ≈ `video.duration`**(실측 977.92 / 980, 245.32 / 247, 205.02 / 206).
  크게 모자라면 부분 수확이다
...
⚠️ **`평가하기`에서는 `›`도 통하지 않는다 — 그리고 모달조차 뜨지 않는다**(2026-09-09 GitHub 2주차 실측).
퀴즈를 풀지 않은 상태에서 `›`를 누르면 **아무 반응 없이 그 자리에 머문다.** 잠금 모달이 뜨는 다른 경우와
달라서 **클릭이 빗나간 것으로 오인하기 쉽다** — 헤더(`… | 평가하기`)가 그대로인지로 판정하고,
좌표를 의심해 반복 클릭하지 말 것.

→ **`정리하기` 채록은 퀴즈 풀이에 막힌다.** 정답·해설은 클릭 없이 이미 회수되므로(6단계),
  **풀이는 사용자에게 넘기고 `## 학습정리`는 「미채록」으로 표시**한 뒤, 사용자가 푼 다음에 채운다.
...
⚠️ **공식 정답이 강의 내용과 어긋날 수 있다 — 공식 값이 정본이다.**
GitHub 2주차 Q3 「Git을 설치한 후 환경변수를 설정해야 한다」의 공식 정답은 **○**인데,
자막은 「설정은 이름과 전자 우편 두 가지만」이라고 말한다. 자막으로 추론하면 ×가 나온다.
**추론값과 공식값이 다르면 공식값을 쓰고, 그 불일치를 스크립트에 함께 적는다.**
