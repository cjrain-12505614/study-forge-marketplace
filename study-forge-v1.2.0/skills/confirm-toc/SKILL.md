...
description: >
  ...
  ## (N) 순번 매칭을 근거 있게 확정하는 스킬 (v0.10.0).
...
- **완료 주차의 학습평가** — 스크립트에 공식 정답(채점 기록)이 없으면 `평가하기`를 열고 **헬퍼를 먼저 (재)정의**한 뒤 `__scuQuiz.res(k)`(또는 `probe()`·`next()`)로
  **채점 상태부터 읽는다**(옛 헬퍼는 교체된 문서를 계속 본다 — `probe().stale`). 이미 채점돼 있으면 읽기만 하고(`save(k)`), 초기화돼 있으면 채점 루프를 탄다
  (재오픈 시 초기화는 **1회 관측이라 미검증** — 본 결과를 8단계에 적는다).
...
4. `selNo(k)`가 `null`이면 **합성 이벤트를 반복하지 말고 실제 마우스 클릭으로 전환**한다 —
   ...
   - ⚠️ 탭이 `visibilityState: hidden`이면 문항 rect가 **0**으로 나왔다가 스크린샷(탭 활성화) 뒤 정상화됐다 — rect 0을 「문항 없음」으로 읽지 않는다
...
window.__scuQuiz = (function () {
  ...
  const qText   = k => { const q = it_(k) && it_(k).querySelector('[data-field="question"]'); return clean(q && q.textContent); };
  ...
  const guard = k => stale() ? '중단 — 플레이어 문서가 교체됨(재오픈·항목 전환) — 헬퍼 재정의'
    : hidden()   ? '중단 — 탭 hidden (rect 0 · 유휴 팝업·좌표 판정 불확실) — 스크린샷으로 활성화 후 probe()부터'
  ...
  return {
    ...
    probe: () => {
      ...
      return {
        ...
        idle: idle(), tab: document.visibilityState, saved: Object.keys(store()).length
      };
    },
    ...
    scan: (a, b) => {
      ...
      for (let k = from; k < to; k++) {
        const it = it_(k), q = it.querySelector('[data-field="question"]');
        const rec = {
          no: k + 1, type: type(k), q: clean(q && q.textContent),
    ...
    coordConf: k => {
      ...
      if (hidden()) return '중단 — 탭 hidden — 스크린샷으로 활성화 후 probe()부터';
    ...
    next: () => {
      ...
      if (hidden()) return { step: 'activate', why: '탭 hidden — 스크린샷으로 활성화 후 probe()', guard: gd };
...
0. **준비** — **조작 전 스크린샷으로 탭을 앞으로 가져온다**(hidden이면 rect 0 · 헬퍼가 거부한다) → `__scuIdle()`이 `null`인지, `__scuNS`가 설정됐는지, 헤더가 `… | 평가하기`인지 확인 → 헬퍼 정의 → `probe()`로 확인한다
   - `tab`이 `visible`이 아니면 **조작하지 않는다** — hidden 탭은 rect가 0이라 유휴 팝업·좌표 판정이 불확실하다. 스크린샷으로 활성화한 뒤 `probe()`를 다시 부른다
...
- ⚠️ 재오픈 뒤 채점 상태가 **유지되는지·초기화되는지는 확정되지 않았다**(초기화 관측 1회 — 위 「⭐ 공식 정답의 정본」). 본 결과를 8단계 보고에 적어 사실을 누적한다
