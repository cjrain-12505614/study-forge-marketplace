...
#### ⭐ 정답은 텍스트가 아니라 DOM에 있다 — 정규식보다 이쪽이 정본이다

**2026-09-09 문제해결프로그래밍입문 2주차 실측 — 채점 결과가 클래스·속성으로 남는다.**

| 대상 | 근거 | 의미 |
|---|---|---|
| 정답 보기 | `.item-option-row`에 **`passed-answer`** 클래스 | 그 행이 **정답**이다 |
...
[...d.querySelectorAll('[data-type="item"]')].map((it, k) => ({
  k: k + 1,
  passyn: it.getAttribute('data-passyn'),                       // Y/N — 정오
  q: (it.querySelector('[data-field="question"]')?.textContent || '').replace(/\s+/g, ' ').trim(),
  rows: [...it.querySelectorAll('.item-option-row')].map(r => ({
    no:      r.querySelector('.item-select-option-no')?.textContent,
    label:   (r.textContent || '').replace(/\s+/g, ' ').trim(),
    correct: r.classList.contains('passed-answer'),             // ⭐ 정답 표시
    picked:  !!r.querySelector('.item-select-option-no.selected'),
  })),
  fields: [...it.querySelectorAll('.component-data-field')]
            .map(e => (e.textContent || '').replace(/\s+/g, ' ').trim()).filter(Boolean),
}))
```

- ⛔ **두 마커 모두 `정답확인`을 눌러야 생긴다** — 누르기 전 문항에는 `passed-answer`도 `data-passyn`도 **없다**(실측: 1번만 누른 상태에서 2~6번 전부 부재). **미리 읽어 정답을 알아낼 수는 없다**
- 정답을 **틀리게 골라도** `passed-answer`는 정답 행에 붙으므로, 정오와 무관하게 공식 정답을 회수할 수 있다
- 해설은 그 문항 컨테이너의 마지막 `.component-data-field`로 나온다 — 보기·문제 텍스트를 제외하고 남는 것이 해설이다
- ⭐ **이 경로를 우선 쓰고, 위 정규식은 폴백으로 둔다.** 정규식은 `innerText` 레이아웃에 의존해 라벨 표기가 바뀌면 깨지지만, 클래스·속성은 컴포넌트 계약이라 안정적이다
- ⚠️ `innerText`는 채점된 문항의 버튼을 **숨겨서 빼버린다** — 텍스트만 보면 「버튼이 사라졌다」로 보이지만 `querySelectorAll`에는 그대로 잡힌다(가시성만 `display:none`). **텍스트 유무로 채점 여부를 판정하지 말 것**
