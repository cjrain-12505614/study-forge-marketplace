...
description: >
  ...
  10단계에서 `lint_note()`로 제목 골격·서식·금지 표현·그림 참조·원본 해시를 기계 점검한다 (v0.13.0).
version: 0.13.0
---
...
| **서식·표기·원본** | 제목 골격 = 학습목차 · `### N-k` 번호 · 줄 끝 마침표 0 · 코드블록 최상위 · 표 열 수 · 그림 참조 유효 · 쪽 표기/작업 메모/메일 주소 0 · 원본 해시 불변 | `lint_note()`로 **기계 점검** (아래) — 읽으며 확인하는 것은 보조 |
...
6. **노트 기계 점검**: `lint_note(노트, toc=…, img_dir=…, sources=…)`의 `fails`가 비어 있어야 한다 (아래 코드)
   — `warns`(참조되지 않은 그림 · `#` 앞 구분선)는 사람이 판단한다
7. **서식 최종 점검**: 기계가 보지 못하는 것 — 개조식 문체, 코드블록 언어 태그, 표 정렬, 설명의 앞뒤 순서
...
### 노트 기계 점검 — 제목 골격·서식·금지 표현·원본 불변

학습평가 대조가 끝나면 **노트 전체를 기계로 한 번 훑는다.** 사람이 읽어 넘기기 쉬운 것(줄 끝 마침표, 들여쓴 코드블록, 표 열 수, 없는 그림 참조, 작업 메모 누출)을 잡고, **원본(스크립트·교안·자막·앞 주차 노트)이 작업 중에 바뀌지 않았는지**까지 확인한다.

- `toc` — `parse_outline(script_content)`의 제목 골격(`[(1, '들어가기'), (2, '(1)학습개요'), …]`). 주면 노트 `#`/`##` 제목이 학습목차와 같은지 본다. 부록은 골격 뒤에 오는 `##`로 보고 통과시킨다
- `img_dir` — 과목 `강의노트/img/`. 주면 `![](img/…)` 참조가 실재하는지, 참조되지 않은 이번 주차 그림이 있는지 본다(`img_prefix`는 `01_`처럼 주차 접두)
- `sources` — `{'스크립트': (경로, sha256), …}`. 작업 시작 때 찍어 둔 해시와 비교한다

⚠️ **오탐을 막으려고 일부러 좁혀 둔 곳** (2026-09-20 15개 노트 실측으로 확정)

| 검사 | 제외 | 이유 |
|---|---|---|
| 작업 메모 「프레임」 | `프레임워크` · `프레임 워크` | 교안·강의의 정상 용어다 — 「프레임 227장」·「판독」은 그대로 잡는다 |
| 줄 끝 마침표 | `## 학습평가` 절의 보기 줄(`1. …` · `- ○`) | 공식 문항 원문이라 고치면 안 된다 |
| 쪽 표기 | 「3페이지짜리」처럼 뒤에 한글이 붙는 말 | 쪽 번호 인용이 아니다 |
...
LINT_FENCE = '`' * 3  # 코드 펜스 — 이 문서의 코드블록을 끊지 않도록 리터럴로 쓰지 않는다

NOTE_LINT_FORBIDDEN = [
    (r'\bp\.\s?\d|\bp-\d{1,2}\b', '쪽 표기'),
    (r'\d+\s*(?:쪽|페이지)(?![가-힣])', '쪽 표기'),
    (r'슬라이드\s*\d+\s*(?:번|쪽|페이지)', '슬라이드 번호'),
    (r'※|←', '주석 기호'),
    # 「프레임워크」·「프레임 워크」는 교안 용어라 통과시킨다 (2026-09-20 빅데이터 1·3주차 오탐)
    (r'채록|판독|프레임(?!\s?워크)|digest|에이전트', '작업 메모'),
    (r'v\d\d_frame|\.(?:mp3|vtt)\b', '작업 파일명'),
    (r'다글로|전사 본문|옛 노트', '작업 메모'),
    (r'passed-answer|failed-answer|passyn|data-(?:field|passyn)|pageindex', 'DOM 메모'),
    (r'(?<![\w.+-])[\w.+-]+@(?!email\.com|example\.)[\w-]+\.(?:ac\.kr|com|net|org|edu)\b', '메일 주소'),
]


def _lint_outside(note_md):
    """(행번호, 줄, 줄 시작 오프셋) — 코드블록 안은 건너뛴다"""
    fence, off = False, 0
    for i, ln in enumerate(note_md.split('\n'), 1):
        start = off
        off += len(ln) + 1
        if ln.lstrip().startswith(LINT_FENCE):
            fence = not fence
            continue
        if not fence:
            yield i, ln, start
...
def _lint_quiz_range(note_md):
    """`## 학습평가` 절의 (시작, 끝) 오프셋 — 없으면 None"""
    m = re.search(r'(?m)^## 학습평가\s*$', note_md)
    if not m:
        return None
    nxt = re.search(r'(?m)^#{1,2} (?!#)', note_md[m.end():])
    return m.start(), m.end() + (nxt.start() if nxt else len(note_md) - m.end())


def lint_note(note_md, toc=None, img_dir=None, img_prefix='', sources=None):
    """강의노트 기계 점검 — {'fails': [...], 'warns': [...]}

    fails 가 비어 있어야 저장(9단계)·보고(10단계)로 넘어간다.
    """
    fails, warns = [], []
    lines = list(_lint_outside(note_md))
    quiz = _lint_quiz_range(note_md)

    # 1. 제목 골격 — 학습목차 그대로인가 (부록 `##`는 골격 뒤에 온다)
    heads = [(len(m.group(1)), m.group(2).strip())
             for _i, ln, _o in lines for m in [re.match(r'^(#{1,2}) (.+?)\s*$', ln)] if m]
    if toc:
        want = [(lv, tx) for lv, tx in toc]
        got = heads[:len(want)]
        if got != want:
            bad = [(g, w) for g, w in zip(got, want) if g != w][:3]
            fails.append('제목이 학습목차와 다름 — {}'.format(bad or '개수 부족'))

    # 2. `### N-k.` 소제목 — 구역 번호가 맞고 1부터 연속인가
    cur, secs = None, {}
    for i, ln, _o in lines:
        m2 = re.match(r'^## \((\d+)\)', ln)
        if m2:
            cur = int(m2.group(1))
            secs[cur] = []
        ...
        if re.match(r'^##? ', ln):
            cur = None
            continue
        m3 = re.match(r'^### (\d+)-(\d+)\.', ln)
        if m3:
            if cur is None or int(m3.group(1)) != cur:
                fails.append('{}행 소제목 번호 {} 가 구역과 다름'.format(i, m3.group(0)))
            else:
                secs[cur].append(int(m3.group(2)))
    for n, ks in secs.items():
        if ks != list(range(1, len(ks) + 1)):
            fails.append('`### {}-k` 번호가 연속이 아님 — {}'.format(n, ks))

    # 3. 줄 끝 마침표 — 학습평가 보기 줄은 공식 원문이라 제외
    dots = [(i, ln) for i, ln, o in lines
            if re.match(r'^\s*(?:[-*]|\d+\.) ', ln) and ln.rstrip().endswith('.')
            and not re.search(r'(?:\.\.\.|\d\.)$', ln.rstrip())
            and not (quiz and quiz[0] <= o <= quiz[1]
                     and re.match(r'^\s*(?:\d+\.|[-*] ?[○×OX])\s', ln))]
    if dots:
        fails.append('줄 끝 마침표 {}곳 (예: {}행 {})'.format(len(dots), dots[0][0], dots[0][1].strip()[:40]))

    # 4. 코드블록 — 들여쓰기 금지 · 펜스 짝
    indented = [i for i, ln in enumerate(note_md.split('\n'), 1) if re.match(r'^\s+' + LINT_FENCE, ln)]
    if indented:
        fails.append('들여쓴 코드블록 {}행'.format(indented[:3]))
    if note_md.count(LINT_FENCE) % 2:
        fails.append('코드블록 펜스 짝이 맞지 않음')

    # 5. 표 — 머리 구분줄 · 열 수 · 표 앞 빈 줄
    all_lines = note_md.split('\n')
    blocks, cur_b = [], []
    ...
    if cur_b:
        blocks.append(cur_b)
    for b in blocks:
        cols = b[0][1].replace('\\|', '').count('|')
        odd = [i for i, ln in b if ln.replace('\\|', '').count('|') != cols]
        if odd:
            fails.append('표 열 수 불일치 {}행'.format(odd[:3]))
        if len(b) < 2 or not re.match(r'^\|[-| :]+\|$', b[1][1]):
            fails.append('{}행 표에 머리 구분줄 없음'.format(b[0][0]))
        if b[0][0] > 1 and all_lines[b[0][0] - 2].strip():
            fails.append('{}행 표 앞에 빈 줄 없음'.format(b[0][0]))

    # 6. 그림 참조 ↔ 실제 파일
    refs = re.findall(r'!\[[^\]]*\]\(img/([^)]+)\)', note_md)
    if img_dir:
        have = {f for f in os.listdir(img_dir) if f.lower().endswith('.png')}
        miss = [r for r in refs if os.path.basename(r) not in have]
        if miss:
            fails.append('없는 그림을 참조함 — {}'.format(miss))
        if img_prefix:
            orphan = sorted(f for f in have if f.startswith(img_prefix) and f not in refs)
            if orphan:
                warns.append('참조되지 않은 이번 주차 그림 — {}'.format(orphan))
    bad_ext = [r for r in refs if not r.lower().endswith('.png')]
    if bad_ext:
        fails.append('PNG가 아닌 그림 참조 — {} (SVG 직접 참조 금지)'.format(bad_ext))

    # 7. 금지 표현 — 출처·작업 메모·개인정보가 노트에 새지 않았는가
    for pat, why in NOTE_LINT_FORBIDDEN:
        hit = [(i, ln.strip()[:50]) for i, ln, _o in lines if re.search(pat, ln)]
        if hit:
            fails.append('{} {}곳 — {}행 「{}」'.format(why, len(hit), hit[0][0], hit[0][1]))

    # 8. 구분선 — `#` 앞에 하나(경고) · 연속 금지(오류)
    #    학습목차가 `---` 없이 `#`를 잇는 과목이 있어 앞 구분선은 경고로만 본다
    hr_at = [i for i, ln, _o in lines if ln.strip() == '---']
    h1_at = [i for i, ln, _o in lines if re.match(r'^# ', ln)]
    for i in h1_at[1:]:
        prev = [x for x in hr_at if x < i]
        if not prev or any(all_lines[j - 1].strip() and not all_lines[j - 1].startswith('#')
                           for j in range(prev[-1] + 1, i)):
            warns.append('{}행 `#` 앞에 구분선 없음'.format(i))
    if re.search(r'(?m)^---\n\s*\n---$', note_md):
        fails.append('구분선이 연속으로 두 번')

    # 9. 원본 불변 — 작업 중 스크립트·교안·자막이 바뀌지 않았는가
    for name, (path, want_hash) in (sources or {}).items():
        try:
            with open(path, 'rb') as fh:
                got = hashlib.sha256(fh.read()).hexdigest()
        except OSError:
            fails.append('원본을 열 수 없음 — {} ({})'.format(name, path))
            continue
        if got != want_hash:
            fails.append('원본이 바뀜 — {}'.format(name))
...
```python
toc = []
for it in (parse_outline(script_content) or []):          # 3-A 블록
    if it['type'] == 'h1':
        toc.append((1, it['text']))
    elif it['type'] == 'h2':
        toc.append((2, it['text']))
    elif it['type'] == 'h2_numbered':
        toc.append((2, '({}){}'.format(it['num'], it['text'])))

res = lint_note(
    open(note_path, encoding='utf-8').read(),
    toc=toc,
    img_dir=os.path.join(note_dir, 'img'), img_prefix='{:02d}_'.format(week),
    sources={'스크립트': (script_path, script_sha256)})
print('통과' if not res['fails'] else f"FAIL {len(res['fails'])}")
for f in res['fails']:
    print('  ✗', f)
for w in res['warns']:
    print('  ·', w)
...
> **실측 (2026-09-20 · 5과목 1~3주차 노트 15개, 읽기 전용)** — 15개 모두 `fails` 0. 좁히기 전 패턴으로는 3개 노트에서 오탐 4건이 났다(빅데이터 1·3주차의 「프레임 워크」·「프레임워크」 4건, GitHub 2주차 학습평가 보기 줄 마침표 12건). 일부러 넣은 위반(본문 불릿 마침표 · 「프레임 227장을 판독」)은 그대로 잡힌다
...
- 기계 점검: ✓ (lint_note fails 0 · warns N)
- 서식: ✓
- 보강 사항: 없음 / N건 보강 완료
```
